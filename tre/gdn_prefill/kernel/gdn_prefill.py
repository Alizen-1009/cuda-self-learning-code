...
def chunk_gated_delta_rule_sm100(
    ...
    mn_mode: bool = False,
    m_seg: Optional[torch.Tensor] = None,
) -> None:
    ...
    # Stage 3 fused MN-precompute: m_seg (S_hom -> M_seg) is the 2nd state output.
    _m_seg = m_seg if mn_mode else None
    ...
    cache = _get_compiled_cache(
        ...
        enable_checkpoints,
        mn_mode,
        os.environ.get("GDN_MN_FULL", "1"),  # [STAGE3 debug] bisection toggle
        os.environ.get("GDN_MN_SYNC", "1"),  # [STAGE3] serialization-barrier A/B
    )
    ...
    if "compiled" not in cache:
        ...
        # Stage 3: homogeneous-state output M_seg (same [N,HO,DK,DK] layout as s_out).
        m_seg_cute = None
        if mn_mode:
            m_seg_cute = from_dlpack(_m_seg, assumed_align=16)
            m_seg_cute.mark_layout_dynamic().mark_compact_shape_dynamic(
                mode=3, stride_order=(0, 1, 2, 3), divisibility=DK
            )
        ...
        cache["max_active_clusters"] = max_active_clusters

    # --- Execute ---
    compiled = cache["compiled"]
    max_active_clusters = cache["max_active_clusters"]
